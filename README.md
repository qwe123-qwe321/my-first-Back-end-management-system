...
一个基于 React 19 + TypeScript + Vite 构建的现代化后台管理系统，包含完整的前端工程化实践。项目数据内容以游戏角色为主题，展示了丰富的后台管理场景。
...
### 数据可视化
- **ECharts 仪表盘** - 展示趋势图、分布图、排行榜等多种图表
- **英雄档案管理** - 角色详情、皮肤切换、语音试听等多媒体管理功能
- **社区动态展示** - 模拟社交平台内容流展示
...
├── components/          # 组件
│   ├── ui/            # 基础 UI 组件
│   ├── layout/        # 布局组件（侧边栏、头部、页脚）
│   └── common/        # 通用组件（面包屑、页面过渡、占位符）
├── pages/              # 页面
│   ├── Login/         # 登录页
│   ├── Dashboard/     # 仪表盘首页
│   ├── Heroes/        # 英雄档案管理
...
|------|------|------|
| `/login` | 登录页 | 用户登录入口 |
| `/dashboard` | 仪表盘 | 数据概览与可视化图表 |
| `/heroes/profile` | 英雄档案 | 角色详情、皮肤切换、语音试听 |
| `/heroes/voice` | 语音动画 | 英雄语音展示 |
| `/skins` | 皮肤管理 | 皮肤列表与管理 |
| `/community` | 玩家社区 | 社区动态展示 |
| `/settings` | 个人主页 | 个人信息编辑 |
| `/users` | 用户管理 | 用户 CRUD 操作 |
| `/about` | 关于系统 | 项目介绍 |
| `/error-pages/403` | 403 页面 | 无权限提示 |
| `/error-pages/404` | 404 页面 | 页面未找到 |
| `/error-pages/500` | 500 页面 | 服务器错误 |
